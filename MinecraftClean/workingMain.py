...
def getblocklist(savefile):
    ...
    for block in root.findall('BLOCK'):
        y = int(block.get('X'))
        x = int(block.get('Y')) # was y
        z = int(block.get('Z')) # was z
...
class Model(object):
    ...
    def _initialize(self):
        ...
        blocklist = getblocklist('outputtree.xml')
        for block in blocklist:
            locList = block[0:3]
            mytuple = tuple(locList)
            _type = block[3]
            for texture in TextureList:
                if texture[1] == _type:
                    self.add_block(mytuple, texture[0], immediate = False)
